...
function newGame() {
  ...
  document.getElementById('timeDisplay').innerText = gameTime / 1000;
  ...
  //Fix: delay focus so it works after rendering
  setTimeout(() => {
...
function getAccuracy() {
  const allTypedLetters = document.querySelectorAll(
    ".letter.correct, .letter.incorrect, .letter.extra"
  );
  const correctLetters = document.querySelectorAll(".letter.correct");
  const totalTyped = allTypedLetters.length;
  const totalCorrect = correctLetters.length;
  const accuracy = totalTyped === 0 ? 0 : (totalCorrect / totalTyped) * 100;
...
function gameOver() {
  ...
  const resultAccuracy = getAccuracy();
  document.getElementById("wpmDisplay").innerText = `WPM: ${resultWpm}`;
  document.getElementById(
    "accuracyDisplay"
  ).innerText = `Accuracy: ${resultAccuracy}%`;
}
...
document.getElementById("game").addEventListener("keyup", (ev) => {
  ...
  if (!window.timer && isLetter) {
    window.timer = setInterval(() => {
      ...
      const sPassed = Math.floor(msPassed / 1000); // floor to count 0
      const sLeft = Math.max(0, Math.floor(gameTime / 1000 - sPassed)); // ensure it doesn't go negative

      // Update display
      document.getElementById("timeDisplay").innerText = sLeft;
      const percent = (sLeft / (gameTime / 1000)) * 100;
      document.getElementById("timeBar").style.width = `${percent}%`;

      // Live WPM & Accuracy update
      const resultWpm = getWpm();
      const resultAccuracy = getAccuracy();
      document.getElementById("wpmDisplay").innerText = `WPM: ${resultWpm}`;
  
      const accuracyEl = document.getElementById("accuracyDisplay");
      accuracyEl.innerText = `Accuracy: ${resultAccuracy}%`;

      accuracyEl.classList.remove("accuracy-good", "accuracy-medium", "accuracy-bad");
      const accValue = parseFloat(resultAccuracy);
      if (accValue >= 90) {
      accuracyEl.classList.add("accuracy-good");
      } else if (accValue >= 70) {
      accuracyEl.classList.add("accuracy-medium");
      } else {
      accuracyEl.classList.add("accuracy-bad");
      }
...
gameElement.addEventListener('blur', () => {
  focusError.style.display = 'block';
});

focusError.addEventListener('click', () => {
   gameElement.focus();
 });
